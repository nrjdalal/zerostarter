...
import { HANDOFF_TOKEN_PATTERN, mintHandoffToken } from "@packages/auth/handoff"
...
// Cross-origin session handoff, live in split mode only (two apps whose origins cannot share a cookie domain). After OAuth completes on the api origin, /start parks the session-cookie value under a one-time id and bounces to the web origin, whose /api/handoff route claims it server-to-server and sets the cookie first-party. Storage goes through Better Auth's own verification adapter, the same primitive its one-time-token plugin uses, so there is no bespoke SQL to drift from the framework. Shared and host-only deployments never reach these routes: the mode gate below 404s every request before auth even runs.
