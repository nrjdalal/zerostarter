...
| **P2** ~193KB unused font preloads           | `preload: false` on `caveat`/`newsreader` (`lib/fonts.ts`); only used on `/hire`,`/resume`                                                         | ~193KB off the landing critical path                                         |
...
| **P3** unused-javascript 325KB               | Defer PostHog: dynamic `import("posthog-js")` on idle in `instrumentation-client.ts`; drop the unused root `PostHogProvider` (`app/providers.tsx`) | unused-JS 325 → 246KB (local)                                                |
| **P3** legacy-javascript 54KB                | Modern `browserslist` floor in `web/next/package.json` (Chrome/Edge/FF ≥111, Safari ≥16.4)                                                         | legacy-JS 54 → 13KB (local)                                                  |
| **P5** ~62KB assets without long TTL         | `headers()` in `next.config.ts` for `/marketing/:path*`: `public, max-age=86400, stale-while-revalidate=604800` (covers standalone + Vercel)       | Confirmed via `curl -I`                                                      |
...
Once P4 and the P1 extras are decided, delete this doc per the `.github/audit/` convention.
