...
// The next free port at or after `start`, so several worktrees can `bun run dev` at once (3000/4000, then 3001/4001, ...) without colliding; probes on 0.0.0.0 to match how the dev servers bind (a 127.0.0.1 probe misses a sibling on `*`). The apps bind these via portless --app-port and INTERNAL_API_URL follows HONO_PORT.
