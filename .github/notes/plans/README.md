...
- [WebSockets on Vercel through Bun.serve()](vercel-bun-serve-websockets.md) - the Node adapter split exists because Vercel could not run `Bun.serve()`; its Bun runtime now can, in beta and with three documented differences, which would let one code path serve every host.
