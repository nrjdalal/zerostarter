...
- Status: planned
- Links: PR #754 (this plan); PR #753 (closed spike PR, full review discussion); working spike on `spike/platform-data-table` (~40 commits of iteration, all requirements below verified live there); shadcn data-table guide; TanStack Table v8 virtualized-infinite-scrolling example

One data table for the whole platform, rebuilt clean from this spec. A spike proved every requirement end to end and is preserved only as history; the rebuild is written from scratch against this document alone. Every requirement below was converged on deliberately, most after seeing the alternative fail.
...
```
components/data-table.tsx         the whole family as ONE module (the shadcn
                                  sidebar.tsx pattern: one file, many exports,
                                  hooks included like useSidebar):
                                  DataTable (the region),
                                  DataTableCellText (truncate | wrap overflow),
                                  DataTableColumnHeader (title + bare sort icon),
                                  DataTableFacetedFilter (Popover + Command),
                                  DataTableToolbar (search + slot + View),
                                  DataTableViewOptions (labels from meta.label),
                                  ColumnConfig + applyColumnManager (layout),
                                  useDataTable (URL + infinite query + table),
                                  useDataTableState (URL layer, client tables)
app/(console)/console/(platform)/users/
  page.tsx                        synchronous server component (no page gate)
  components/data-columns.tsx     this table's columns, fully: the defs (header,
                                  cell, meta.label) plus its ColumnConfig map
...
- One import site for everything table-shaped: `import { DataTable, DataTableToolbar, useDataTable, type ColumnConfig } from "@/components/data-table"`. The module carries `"use client"` + `"use no memo"` once at the top (every part reads table or column instances). Route-colocated files stay role-named (`data-columns.tsx`, `data-table.tsx`); there the page context lives in the export (`UsersDataTable`), not the filename.
...
The module ships the mechanism (`ColumnConfig`, `applyColumnManager`, measurement); each table's config is data that colocates in its own `data-columns.tsx`, exported beside the defs and written in column order. There is no central registry and no `<area>.<table>` namespacing: the file location is the namespace. Config maps column ids onto:
...
- The users config converged to: `select { width: 12 }`, `name { extra: 48 }` (header + 12rem), `email { extra: 60, flex: true }`, `role {}`, `status {}`, `createdAt { align: "right", extra: 24 }`, `actions { align: "center", width: 12 }`. Role and status center; select stays left deliberately (see slack model).
...
- `DataTableCellText` overflow follows the manager's `wrap` flag through column meta (`cell: ({ column }) => <DataTableCellText column={column}>...`), not a per-cell prop: false truncates (one line, ellipsis, tooltip only when actually cut, measured `scrollWidth > clientWidth` on hover/focus), true wraps (`whitespace-normal wrap-break-word`; the measured virtual row grows). Web terminology, not invented terms.
...
3. The single data-table module per this spec.
