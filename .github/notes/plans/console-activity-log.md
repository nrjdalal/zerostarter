...
- Status: in progress
- Links: PR #758 review (raised); graduated from the icebox once the shape was decided
...
One `audit_log` table with a typed event per write, an actor that may be a person or a rule, and a label stored beside every id so a row still reads after its target is deleted (an allowlist rule is gone by the time its removal is read). `Console > Activity`, admin and above, on the same server-driven table as the rest.
