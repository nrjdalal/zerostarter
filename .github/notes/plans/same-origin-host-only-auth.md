...
Two build-inlined `NEXT_PUBLIC_*` values point at the build-time host, not the actual host, and so will misfire under one-build-many-hosts previews (#677): `config.api.url` (the cross-origin health WebSocket) and `config.app.url` (the OAuth `callbackURL` in `access.tsx`). Harmless today (the WS is a public health probe; OAuth needs a real registered host anyway), but both are the same hazard `config.api.base` deliberately dodged by using `window.location.origin`, and preview auth must resolve them when #677 is wired up. Track against #677 / the deferred issue (#707).
