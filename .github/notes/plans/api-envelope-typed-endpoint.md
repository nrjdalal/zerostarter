...
Historical note: the API deliberately moved off hono-openapi's `validator` to `sValidator` early on (PR #287 removed the validator-driven query example; #523 landed real validation on `sValidator`) because an older `hono-openapi` regressed Hono RPC inference, the typed client, the eval's "crown jewel." Before re-adopting the validator here, that was re-verified zero-regression on the current pin: the full `AppType` in `api/hono/dist/index.d.mts` is byte-identical between `sValidator` and `validator`, a 12-case runtime battery (valid/invalid/missing/whitespace-trim/over-length/malformed-JSON/wrong-content-type/honeypot/count) is byte-identical, and the bundle is marginally smaller. Guard-rail: on any `hono-openapi` bump, re-diff `dist/index.d.mts`; if the typed client changed, revert validation to `sValidator` and keep the `requestBody` documented via `jsonRoute`.
