...
- **No platform** walks a generic list, `x-forwarded-for` last hop first and then ten other headers, and the README's own word on the result is that the IP "should not be trusted as it can be spoofed in most cases"; the guards it offers apply only when a `platform` is passed.
