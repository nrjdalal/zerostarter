...
- **No platform** walks a generic list, `x-forwarded-for` last hop first and then ten other headers, and the README does not vouch for the result: its guards apply only when a `platform` is passed (quoted on the rate-limiting docs page).
