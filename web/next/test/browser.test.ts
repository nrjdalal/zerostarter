...
// the waitlist e2e test inserts a real row via the form -> API -> DB; clean it
// up so test:e2e is self-cleaning (only fires when that test could have run)
afterAll(async () => {
  if (enabled && writesEnabled) await cleanupSmokeRows()
})

describe.skipIf(!enabled)("browser behaviors", () => {
  // true end-to-end mutation: drives the actual signup form, not a raw POST.
  // Gated on ALLOW_DB_WRITES since it persists a row (cleaned in afterAll).
  test.skipIf(!writesEnabled)(
    "waitlist: real signup through the form persists and shows the success state",
    async () => {
      await open("/")
      ab("fill", "input[type=email]", `smoke-e2e-${Date.now()}@example.test`)
      clickByName("Join the waitlist")
      // the form swaps to the confirmation copy only after the API accepts it
      await waitFor(`document.body.innerText.includes("You're on the list")`, "waitlist success")
    },
    60_000,
  )
  ...
  test("home: api status and waitlist count widgets resolve client-side", async () => {
    await open("/")
    // ApiStatus flips to operational once /api/health resolves
    await waitFor(
      `document.body.innerText.includes('All systems are operational')`,
      "api status operational",
    )
    // the social-proof widget renders "N+ people on the waitlist" once count loads
    await waitFor(
      `/\\d+\\+ people on the waitlist/.test(document.body.innerText)`,
  ...
  test("login dialog opens, shows providers, validates email", async () => {
    ...
    await waitFor(`document.body.innerText.includes('Welcome to Cafe')`, "login dialog open")
    const d = json(`({
  ...
  test("search dialog: cmd+k opens, type + arrow + enter navigates", async () => {
    ...
    ab("fill", 'input[placeholder="Search..."],input[placeholder="Search"]', "radius")
    // wait for the debounced results to render the radius result link
    await waitFor(
      `[...document.querySelectorAll('a')].some(a => a.getAttribute('href') === '/docs/design-system/foundations/radius')`,
      "search results",
    )
    ab("press", "ArrowDown")
    ab("press", "Enter")
    await waitForPath("/docs/design-system/foundations/radius")
  }, 60_000)

  test("copy-as-markdown affordance is present and clickable", async () => {
    await open("/docs/design-system/foundations/colors")
  ...
  test("docs sidebar: collapsible category expands and navigates", async () => {
    await open("/docs")
    clickByName("Foundations")
    await waitFor(
      `[...document.querySelectorAll('[data-sidebar] a')].some(a => a.textContent.trim()==='Colors')`,
      "Foundations expanded",
    )
    evaluate(
      `[...document.querySelectorAll('[data-sidebar] a')].find(a => a.textContent.trim()==='Colors')?.click()`,
    )
    await waitForPath("/docs/design-system/foundations/colors")
  }, 60_000)
  ...
  test("mobile: sheet menu exposes nav links", async () => {
    ...
    try {
      ...
      const socials = json(
        `[...document.querySelectorAll('a[aria-label]')].filter(a => ['GitHub','Instagram','Reddit','X'].includes(a.getAttribute('aria-label'))).length`,
      )
      expect(socials).toBeGreaterThanOrEqual(4)
      ab("press", "Escape")
    } finally {
  ...
  test("no console errors across pages", async () => {
    for (const path of ["/", "/docs", "/blog", "/blog/hello-world"]) {
      ab("console", "--clear")
      await open(path)
  ...
  test("home: navbar visibility matches mode, devtools widget in dev", async () => {
    await open("/")
    const nav = evaluate(`!!document.querySelector('[aria-label="Main navigation"]')`)
    expect(nav).toBe(MODE === "dev" ? "true" : "false")
    if (MODE === "dev") {
