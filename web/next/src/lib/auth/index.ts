  ...
  api: {
    getSession: async (opts?: { disableCookieCache?: boolean }) => {
      try {
        ...
        // Forward the cookie only. Forwarding every incoming header sends the web app's own Host (and x-forwarded-host) to the api, which platforms route/validate by Host, so the call can come back 403 and read as a lost session. x-forwarded-for is deliberately not forwarded either: at this hop it is client-controlled, and the api's findIp reads the rightmost public IP regardless, so forwarding it cannot fix rate-limit keying and only launders a spoofable value. Cookie-only is the idiom every other server-side api call here uses.
