...
- **[A role ladder + gated `/console`](/docs/getting-started/scripts#console)**: `owner > admin > member > user` on `user.role`, gating the route server-side; manage it from Console > Access, or with `bun run console:roles`.
