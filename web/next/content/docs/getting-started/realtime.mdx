...
The route is a `GET` upgraded with `upgradeWebSocket`, but the socket owner depends on where it runs. Bun (local, Docker, self-host) runs `Bun.serve()`, so the socket comes from `hono/bun`. On Vercel the upgrade goes through the Node adapter (`@hono/node-server` + `ws`), which is the setup [Vercel documents for Hono](https://vercel.com/docs/functions/websockets#node.js-server-frameworks). `api/hono/src/lib/server.ts` picks the adapter at boot from `process.env.VERCEL` and exports the matching server, so `index.ts` only registers routes:
...
<Callout type="info" title="Why two adapters, still">

When this shipped, a Vercel Function could not run `Bun.serve()` at all, which is why the split exists. Vercel's Bun runtime has since gained a [native `Bun.serve()` WebSocket path](https://vercel.com/docs/functions/websockets#bun), with documented differences (headers passed to `server.upgrade()` are not applied, `drain` never fires, `send()` reports no backpressure). The starter has not moved to it: the Node adapter is proven here, and both WebSockets and the Bun runtime are still in beta on Vercel.
