...
`bunx zerostarter reinit` re-scaffolds an **existing git repo** as a fresh ZeroStarter: it deletes every file (keeping `.git` and `.env`, so your history, remote, and local secrets survive), fetches the latest ZeroStarter, rebrands to the directory name, and commits on the current branch. Use it to point an existing repo at ZeroStarter without losing its git history. It refuses a dirty tree, since only committed files are recoverable after the wipe.

## `zerostarter sync`

`bunx zerostarter sync` re-baselines an **existing fork** on ZeroStarter's latest scaffold. A gitpick overlay updates the starter files while your content, `public/marketing`, branding (`site.ts`), `package.json` identity, and favicon are preserved and the files you added are untouched. It requires a clean tree and lands as a reviewable diff you commit yourself (no auto-commit); on any failure it rolls the tree back to your last commit.
