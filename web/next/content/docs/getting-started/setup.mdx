...
A few commands take you from nothing to a running, signed-in app.

<Callout type="info" title="Prerequisites">

Bun is required (the repo pins `bun@1.3.14` via `packageManager` in the root `package.json`). Run the CLI with `bunx`, not `npx`: it shells out to `bunx` to fetch the scaffold and provision Postgres. If Bun is missing it shows how to re-run under Bun (`bunx --bun zerostarter ...`) and offers to install it for you. Docker is optional: `init` uses it to provision a local Postgres automatically, and without it you set `POSTGRES_URL` yourself.
...
- provisions a local Postgres in Docker (via [pglaunch](https://github.com/nrjdalal/pglaunch)), reusing an already-running one when you re-run `init`, and points `.env` at it,
- writes `.env` from `.env.example` with a freshly generated `BETTER_AUTH_SECRET`.

Everything else has a working default. The database step defaults to yes when Docker is running; pass `--db` to provision it without the prompt. The CLI does not install dependencies or run migrations, so finish with `bun install`, `bun run db:migrate`, then `bun run dev` (running your own install keeps it fast and sets up the git hooks).
