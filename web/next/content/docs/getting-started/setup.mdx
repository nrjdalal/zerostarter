...
- **`bunx zerostarter reinit`** re-scaffolds an **existing git repo** as a fresh ZeroStarter, keeping `.git` and your `.env*` files, so your history, remote, and local secrets survive. Any failure rolls the tree back to your last commit.
- **`bunx zerostarter sync`** re-baselines an **existing fork** on the latest starter, updating the shared files while keeping everything your fork owns. It leaves the result as a diff for you to review and commit. A failed overlay rolls back to your last commit; a failed dependency install after it does not, so you re-run `bun install` rather than the whole sync.
