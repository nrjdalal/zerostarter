...
- **`web/next`**: the TanStack Start frontend (file-based routing, React 19).
...
├─ web/next/      # frontend (TanStack Start): src/app routes, components, content/
└─ packages/
...
File-based routes under `src/app`, grouped by concern: `(content)/` for docs and blog, `(protected)/` for the signed-in dashboard, `(console)/` for the admin console, and `(llms.txt)/` for the generated AI endpoints. The typed API client is one file (`src/lib/api/client.ts`), and MDX content lives under `content/docs/` and `content/blog/`.
...
The `(marketing)/` route group under `src/app/` holds the marketing surfaces: the landing page (`/`) plus the author's own `/hire` and `/resume`. They stay in the canonical repo but `zerostarter init` strips the whole group and writes a fresh `app/index.tsx` in its place. A direct clone that skips init can delete `web/next/src/app/(marketing)`, add its own `app/index.tsx`, and remove the "Hire" link in `web/next/src/components/navbar/home.tsx`.
