...
- **`src/index.ts`**: Barrel exporting shared helpers (`getSafeEnv`, `getBuildVersion`, `isLocal`)
...
- **`src/lib/`**: `constants.ts` (the `NODE_ENV` enum) and `utils.ts` (dotenv loader, redaction)
