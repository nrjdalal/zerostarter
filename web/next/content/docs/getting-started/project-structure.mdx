...
Standard App Router, grouped by concern: `(content)/` for docs and blog, `(protected)/` for the signed-in dashboard, `(console)/` for the staff console, and `(llms.txt)/` for the generated AI endpoints. The typed API client is `src/lib/api/client.ts`, which owns the client singleton and re-exports the envelope reader beside it in `src/lib/api/unwrap.ts`, and MDX content lives under `content/docs/` and `content/blog/`.
