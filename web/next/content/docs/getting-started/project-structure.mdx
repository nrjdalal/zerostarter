...
`(protected)/` (dashboard) is auth-gated but intentionally empty: it is where your product goes. `(console)/` (admin) ships one working example, a server-driven users table at `/console/users` (see [Data Tables](/docs/manage/data-tables)), and is otherwise the same empty shell.
