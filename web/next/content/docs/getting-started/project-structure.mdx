...
- **`src/lib/error.ts`**: Centralized `errorHandler` (formats Zod validation and internal errors)
...
- Organization and teams plugin for multi-tenant support, plus the Better Auth OpenAPI plugin
...
Shared build config for the workspace: the TypeScript base (`tsconfig.json`, extended by every package) plus a `tsdown` factory (`definePackageConfig`) that the backend packages' `tsdown.config.ts` files call.
