...
Standard App Router, grouped by concern: `(content)/` for docs and blog, `(protected)/` for the signed-in dashboard, `(console)/` for the staff console, and `(llms.txt)/` for the generated AI endpoints. The typed API client is two files (`src/lib/api/client.ts` for the client itself, `src/lib/api/unwrap.ts` for the envelope reader), and MDX content lives under `content/docs/` and `content/blog/`.
