...
- `bun run check-types`: type-check every workspace, then the repo scripts (`check-types:scripts`, `tsc` over `.github/scripts`) and the test suite (`check-types:tests`, `tsc` over each `tests/**/tsconfig.json`).
