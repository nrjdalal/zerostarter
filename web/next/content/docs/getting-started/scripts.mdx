...
- `bun run test:golden`: run the golden suite in `tests/golden` (every endpoint, route, and interaction, black-box via `bun test` + `fetch` for HTTP and the `agent-browser` CLI for interactions). Needs the dev stack already running (`bun run dev`) and `agent-browser` installed.
