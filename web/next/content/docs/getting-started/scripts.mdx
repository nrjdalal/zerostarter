...
- `bun run test`: run the app test suite in `tests/` (every endpoint, route, and interaction, black-box via `bun test` + `fetch` for HTTP and the `agent-browser` CLI for interactions; layout mirrors the repo). Needs the dev stack already running (`bun run dev`) and `agent-browser` installed.
