...
- `bun run release:version`: print the version decision for the current release window as JSON (the last tag, the tree, what the window earned, and the larger of the two); `--write` moves `package.json` to it. The draft-PR workflow runs this on every push to `canary`; running it by hand only previews.
