...
- `bun run test:golden`: run the golden suite in `tests/golden` (every endpoint, route, and interaction, black-box over HTTP + Chromium); starts the dev stack if it is not already running.
