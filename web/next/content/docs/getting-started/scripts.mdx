...
- **`check-types`** runs three passes: every workspace through Turbo, then `check-types:scripts` (`tsc` over `.github/scripts`), then `check-types:tests` (one `tsc` run per `tests/**/tsconfig.json`).
