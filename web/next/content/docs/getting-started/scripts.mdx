...
- `bun run test:e2e`: the end-to-end suite, every `*.e2e.test.ts` under `tests/`, against a running stack. `E2E_API_URL` and `E2E_WEB_URL` default to the compose ports (`localhost:4000`, `localhost:3000`); `E2E_POSTGRES_URL` unlocks the flows that seed a second account. Golden: every contract response is snapshotted, and `bun test --update-snapshots e2e.test` accepts a deliberate change. Skipped when no stack is named, so `bun run test` stays green without one.
