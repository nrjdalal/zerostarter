...
agent-browser open http://zerostarter.localhost:1355   # the web URL bun run dev prints
...
curl -sS -b cookies.txt "$WEB/api/v1/user"   # now authenticated
```
...
```bash
agent-browser open http://zerostarter.localhost:1355/dashboard
