...
### A fork inherits all of it

`bunx zerostarter init` ships every skill and the starter's own `AGENTS.md` into your fork, rebranded to your project name, so an agent opening your repo gets the same playbook rather than a stub. `bunx zerostarter sync` keeps them current: a skill or a guide you have not edited takes the starter's update, and one you have edited stays yours. See [the CLI](/docs/getting-started/cli#skills-you-have-customized).
