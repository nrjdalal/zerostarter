...
if (error) {
  // error.code is a closed set you can switch on
} else {
...
Inside a TanStack Query `queryFn` or `mutationFn`, reach for `unwrapOrThrow` instead: those expect a rejected promise, and it returns the payload directly.
...
It rejects with an `ApiRequestError` carrying `code` alongside `message`, so a caller can still tell a refusal from a network failure. Reaching for `unwrap` and rethrowing `new Error(error.message)` by hand drops the code, which is what the console tables used to do.

Both live in `web/next/src/lib/api/unwrap.ts`, which imports nothing at runtime so tests can exercise the envelope without constructing the client; `@/lib/api/client` re-exports them and stays the entry to import from.
