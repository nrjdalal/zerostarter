...
On the client, the RPC object exposes `$ws()` on the route. Use `wsApiClient`, not `apiClient`: browser RPC goes same-origin through the `/api` proxy, but a WebSocket can't ride that proxy, so `wsApiClient` targets the API's own origin and the socket connects cross-origin (`http` becomes `ws`):

```ts
import { wsApiClient } from "@/lib/api/client"

const socket = wsApiClient.health.ws.$ws()
