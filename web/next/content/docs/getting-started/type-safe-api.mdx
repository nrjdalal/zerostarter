...
export const v1Router = new Hono<{ Variables: Session }>()
  .use("/*", authMiddleware)
  .get("/session", jsonRoute({/* tags, description, sample, output, auth */}), (c) => {
...
Every documented route (`jsonRoute`, or `describeRoute` for the WebSocket upgrade) shows up in an interactive [Scalar](https://scalar.com) reference at `/api/docs`, with request and response schemas derived from its Zod validators and code samples that default to `hono/client`. The raw spec is at `/api/openapi.json`.
