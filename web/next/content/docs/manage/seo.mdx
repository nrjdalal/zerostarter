...
A fresh ZeroStarter is indexable by search engines and unfurls a rich preview on social the moment you deploy it, no configuration. Four metadata routes, all driven by `packages/config/src/site.ts` and your content, do the work:

- `web/next/src/app/robots.ts` → `/robots.txt`
- `web/next/src/app/rss.xml/route.ts` → `/rss.xml`
...
`rss.xml/route.ts` serves an RSS 2.0 feed of published blog posts at `/rss.xml`, built from the same `getPublishedBlogPosts()` gate the sitemap uses, so an unpublished or draft post can never leak into it. It is `force-static` with `revalidate = 60`. The root layout advertises it via `alternates.types`, so a reader autodiscovers the feed from any page. With the blog [feature](/docs/manage/features) off, the channel is still valid but carries no items.
