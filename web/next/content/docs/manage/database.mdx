...
`bun run auth:schema` writes exactly what the Better Auth CLI emits for the plugins and columns declared in `packages/auth/src/schema.ts`. Three things follow:
...
Add indexes when your own row counts and query plans ask for them, not in advance. Two places where that choice shows:
