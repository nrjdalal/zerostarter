...
- **`auth.ts`**, the Better Auth tables: `user`, `session`, `account`, `verification`, plus `organization`, `member`, `team`, `teamMember`, and `invitation`, with their Drizzle relations and foreign-key indexes. See [Auth & Organizations](/docs/manage/authentication) for what each one holds. This file is generated, never edited: `bun run auth:schema` writes exactly what the Better Auth CLI emits for the plugins and columns declared in `packages/auth/src/schema.ts` (the CLI loads `packages/auth/auth-schema.config.ts`, which wraps that declaration in an instance with no database), and a test fails when the committed file differs from a fresh generation (line endings aside, so a Windows checkout passes). A column of your own on `user` or `session` is an `additionalFields` entry there (the console's `roleSetAt` is one), regenerated and then migrated like any other change. The generator runs under Node on purpose: Bun prints `() => new Date` for a function's text, which is the check the CLI makes before it emits a database default, so a Bun run silently drops every `defaultNow()`.
