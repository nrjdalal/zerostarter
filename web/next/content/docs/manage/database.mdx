...
The auth and organization schema is defined in `packages/db/src/schema/auth.ts` (10 tables); the public waitlist uses a standalone `waitlist` table in `packages/db/src/schema/waitlist.ts`.
...
| `passkey`      | WebAuthn credentials      | `id`, `name`, `publicKey`, `userId`, `credentialID` (unique), `counter`, `deviceType`, `backedUp`, `transports`, `aaguid`         |

The `role`/`banned`/`banReason`/`banExpires` columns on `user` and `impersonatedBy` on `session` are provided by the Better Auth Admin plugin and are part of the baseline schema. The `passkey` table is provided by the Better Auth passkey plugin.
...
| `0002_material_scorpion.sql`   | Adds the `passkey` table (Better Auth passkey plugin / WebAuthn credentials), with a `userId` foreign key (cascade) and index                                                                  |
| `0003_shiny_silver_surfer.sql` | Adds a unique index on `passkey.credential_id` (enforces one row per WebAuthn credential)                                                                                                      |
