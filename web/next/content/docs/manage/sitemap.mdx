...
- Published blog posts from `blogSource`
...
- **Blog**: Published posts from `blogSource`, excluding the blog index page (priority 0.9, monthly updates)

Blog entries use `updatedAt` for `lastModified` when present, otherwise `publishedAt` or `createdAt`.
