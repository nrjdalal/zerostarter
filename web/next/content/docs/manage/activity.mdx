...
| Field       | Holds                                                                                                             |
| ----------- | ----------------------------------------------------------------------------------------------------------------- |
| `action`    | what happened, one of a small union: `role.change`, `user.ban`, `user.unban`, `allowlist.add`, `allowlist.remove` |
| `actor`     | who did it, as text                                                                                               |
| `actorId`   | the acting account, when there was one                                                                            |
| `createdAt` | when                                                                                                              |
| `summary`   | the one line the table shows, already written for a reader                                                        |

The table renders `createdAt` as a relative time (`5 min. ago`), and `action` through a label (`Set role`), so the column reads as English while the row keeps the stored code.

`summary` is prose, not a payload. There is no `before`/`after` JSON, no `target_type`, no `metadata` column, because a flat table you can read at a glance is the thing being built here. A role change reads `ada@example.com, member to admin`; the change is in the sentence.
...
// 3. record it inside the transaction doing the work
import { recordActivity } from "@packages/db/console"
