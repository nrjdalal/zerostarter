...
| Set role     | `Confirmed ada@example.com at member` (set to the rung it already had: the rung does not move, but `role_set_at` is stamped, which is the decision worth recording) |
