...
Fonts are self-hosted, not fetched at runtime. `.woff2` files in `web/next/src/fonts/` are declared as `@font-face` rules in `web/next/src/app/fonts.css`, each exposing a CSS variable that `web/next/src/lib/fonts.ts` maps to a Tailwind utility:
