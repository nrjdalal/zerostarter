...
Runs before `git push` (`.github/scripts/ensure-remote-main.ts`). On a fresh fork's first push it publishes `main` and `canary` to the remote and sets `canary` as the default branch via `gh` (a fork's Actions token cannot create branches), so a single `git push origin canary` is enough for the `canary` to `main` release flow to start. It then prints the link to enable read-write Actions permissions (the one thing it cannot set for you), and a local per-remote `git config zerostarter.mainSeeded.<remote>` marker makes it a no-op on every later push.
