...
Runs before `git push` (`.github/scripts/ensure-remote-main.ts`). On a fresh fork's first push it pushes `main` to the remote when it is missing, so a single `git push origin canary` is enough for the `canary` to `main` release flow to start (`main` must exist on the remote, because a fork's Actions token cannot create a branch). It then prints the link to enable read-write Actions permissions, and a local `git config zerostarter.mainSeeded` marker makes it a no-op on every later push.
