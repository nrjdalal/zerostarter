...
- `fetchPage` receives `{ filters, page, perPage, search, sorting }` and returns `{ rows, page, total, hasNextPage }`, passing the endpoint's own paging fields straight through; the console maps column ids onto the endpoint's sort whitelist through a `satisfies`-checked record, so a server rename becomes a compile error, and the row type derives from `InferResponseType`.
