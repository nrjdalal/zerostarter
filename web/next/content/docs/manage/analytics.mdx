...
<Callout type="info" title="Previews count against your allocation">

Every deployment reports, previews included, and Vercel [meters Speed Insights by event](https://vercel.com/docs/speed-insights/limits-and-pricing): the free tier shares 10,000 events over the last 30 days across the whole team, and pauses ingestion for 14 days once a team goes past it. Narrowing to `process.env.VERCEL_ENV === "production"` drops PR previews, but it drops `canary` too, since canary deploys as a preview. To keep canary and lose the rest, test the branch as well, the way `.github/scripts/migrate-on-deploy.ts` already picks its deploy branches:
