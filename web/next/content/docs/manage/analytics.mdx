...
The snippets below are generic PostHog SDK guidance for instrumenting your own product. The starter wires up initialization and the React provider for you, but it does not call `capture`, `identify`, or read feature flags anywhere by default: add those where they make sense in your code.
