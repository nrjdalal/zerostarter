...
`waitlist` in `packages/config/src/site.ts` governs the public page, its API, and the console surface together. With it off, `/waitlist` and `/api/waitlist` return `404`, `/console/waitlist` renders the console's not-found page (with a `200`, like every disabled console page), the nav drops the group, and a fresh fork's home becomes a plain landing page. See [Features](/docs/manage/features).
