...
That is fine for thousands and slow for hundreds of thousands. When it stops feeling instant, the two fixes are independent: a `pg_trgm` GIN index on `waitlist.email` makes the search use an index, and keyset paging replaces the offset. Neither is worth doing before your own numbers ask for it. See [Database](/docs/manage/database).
