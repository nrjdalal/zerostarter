...
Some routes act on rows the caller picked rather than on one resource: banning a selection, changing a role across it, removing several rules. Those take their ids in the **body**, and only those:
...
The single-resource routes keep their path parameter (`/users/:id/role`), because that is what names the thing being changed. A set is not a resource, so it has no id to put there.
