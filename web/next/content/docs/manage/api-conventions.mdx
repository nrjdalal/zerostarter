...
DELETE /api/v1/admin/waitlist       { ids: string[] }
```
...
The OpenAPI document publishes the envelope once, as `components.schemas.Error` (and `ValidationError` for the 400 that carries `issues`), and every 4xx/5xx response references it, so a generated client types one error shape instead of an inline copy per operation. Both come from `api/hono/src/lib/error-schema.ts`, the same Zod schemas the handler validates against.
