...
Every request passes the global middleware in order (from `api/hono/src/index.ts`): **CORS** (origins from `HONO_TRUSTED_ORIGINS` as a strict allowlist in production, plus `*.<base-domain>` subdomains outside production, credentials on) → **logger** → **rate limiter**. Protected routes add auth on top.
