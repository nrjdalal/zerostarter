...
Flat siblings rather than a nested object, which is what Django REST Framework and Laravel answer with and what a reader expects. Stripe and GitHub go further and omit the total, because on a large table counting is the expensive half; these tables are staff sized, the console shows the count, so it stays.

Every key in a documented response runs A→Z, `id` among them rather than hoisted, so one rule covers every shape and a reader never has to learn a second.

Paging is by offset (`?page=2&perPage=25`), which can skip or repeat a row if rows shift between requests. A keyset cursor is the fix and is tracked in the Icebox; the shape above is additive, so a `nextCursor` can join it without moving anything.
...
```json
{
  "data": {
    "results": [
      ...
      { "id": "c", "ok": false, "code": "NOT_FOUND", "message": "User not found" }
    ]
  }
}
```
