  ...
  "data": {
    "results": [
      { "id": "a", "ok": true },
      {
        "code": "FORBIDDEN",
        "id": "b",
        "message": "You can only ban people below your own role.",
        "ok": false
      },
...
The console middleware (on `/api/v1/admin/*`, stacked after `authMiddleware`) re-reads the session with `disableCookieCache: true` and returns `403 FORBIDDEN` unless the caller clears a rung of the [role ladder](/docs/manage/authentication).

- **It requires `admin` throughout.** Every route under it is a console surface for admins: who reaches the console, the rules that let them, the trail of those changes, or the waitlist.
- **It is one instance of a factory** taking the minimum rank, so a future surface for a lower rung states what it needs rather than repeating the rule.
- **The fresh read is the point.** Better Auth's cookie cache holds a session snapshot for ~5 minutes, so without it a revoked admin could keep hitting privileged routes until the cache expired. This mirrors the freshness of the `/console` page gate (see [Authentication](/docs/manage/authentication)).
- **It costs one uncached session read per admin request**, on top of the cached one `authMiddleware` already did, so an admin surface that fetches in batches (an infinite-scroll table does) pays it per batch. That is the price of a revocation taking effect on the next request rather than up to five minutes later.

Routes that serve admin data get `forbiddenErrorResponses` from `lib/error.ts` in their OpenAPI spec.
