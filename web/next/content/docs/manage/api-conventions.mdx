...
`adminMiddleware` (on `/api/v1/admin/*`, downstream of auth) returns `403 FORBIDDEN` unless the session user's `role` is `admin`, the same rule the `/console` gate uses. Routes behind it document the 403 by spreading `forbiddenErrorResponses` into their OpenAPI responses.
