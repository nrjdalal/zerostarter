...
For a one-off route (no source), use `ImageResponse` directly. See `web/next/src/app/og/home/route.tsx` as a reference. For a parameterized preview driven by query params, use the generic `web/next/src/app/og/route.tsx` endpoint (as the hire and résumé pages do).
