...
Because `main` is pushed first, GitHub makes it the default branch. Set the default to `canary` (**Settings → General → Default branch**, or `gh repo edit --default-branch canary`) so new pull requests target `canary`.
