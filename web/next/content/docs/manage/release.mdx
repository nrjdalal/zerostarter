...
This part is the canonical repo's alone: a fork does not take `packages/cli` or `cli-release.yml`. The `zerostarter` npm package has no version of its own (its `package.json` holds a `0.0.0` placeholder); `cli-release.yml` stamps one at publish time, from three triggers:

| Trigger                                                                   | Publishes                         | npm tag  |
| ------------------------------------------------------------------------- | --------------------------------- | -------- |
| `auto-release.yml` finishes                                               | the release version, from the tag | `latest` |
...
The first two run the whole suite on Linux, macOS and Windows before anything is published. The `/release` comment skips that matrix, since its point is a build you can try in seconds: `bunx zerostarter@0.0.0-<short sha> init`.
