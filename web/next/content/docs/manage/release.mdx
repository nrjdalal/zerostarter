...
Mark the draft ready, review it, and merge with a **merge commit**. That merge is what triggers `auto-release.yml`. The release PR's head is a commit the workflow pushed, so GitHub holds its build for approval rather than running it: approve that run under the PR's checks (or `gh api -X POST repos/<owner>/<repo>/actions/runs/<id>/approve`) and let it pass before you merge.
