...
A fork from `zerostarter init` starts with `canary` (one commit ahead) and `main` locally. On your first `git push origin canary`, a pre-push hook (`.github/scripts/ensure-remote-main.ts`, wired in `lefthook.yml`) publishes both `main` and `canary` (a fork's Actions token cannot create branches) and sets `canary` as the default branch via `gh`. It is a no-op once `main` exists on the remote.
