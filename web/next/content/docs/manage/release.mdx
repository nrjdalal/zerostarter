...
<Callout title="Fresh forks: just push canary">

A fork from `zerostarter init` starts with `canary` (one commit ahead) and `main` locally. A pre-push hook (`.github/scripts/ensure-remote-main.ts`, wired in `lefthook.yml`) pushes `main` to the remote on your first `git push origin canary`, because the Actions token cannot create a branch on a fork. It is a no-op once `main` exists on the remote.
