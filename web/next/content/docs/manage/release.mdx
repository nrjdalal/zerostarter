...
<Callout type="warn" title="Required GitHub settings">

A fresh fork's Actions token is read-only by default. Under **Settings → Actions → General → Workflow permissions**, enable both:

- ✅ **Read and write permissions**: lets the workflows create branches, commit the changelog, and tag releases.
- ✅ **Allow GitHub Actions to create and approve pull requests**: lets `auto-canary-into-main` open the release PR.

Without read-write permissions the Actions token gets `403 Resource not accessible by integration` and no release PR opens.

</Callout>

<Callout type="warn" title="Fresh forks: push both branches">

A fork from `zerostarter init` starts with `canary` (one commit ahead) and `main` locally. Push them together so `main` exists on the remote, because the Actions token cannot create a branch on a fork:

`git push origin canary main`

If you push `canary` alone first, the release PR opens only on the next `canary` push or a manual `auto-canary-into-main` run.
