...
- **`@packages/env/api-hono`**: `BETTER_AUTH_URL`, `HONO_PORT`, the rate-limit knobs, `HONO_TRUSTED_ORIGINS`, and the optional `AGENT_SIGNIN_ENABLED` (gates the local agent sign-in)
- **`@packages/env/auth`**: `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `HONO_TRUSTED_ORIGINS`, and the optional OAuth pairs
...
<Callout type="info" title="Upgrading a fork: HONO_APP_URL is now BETTER_AUTH_URL">

`HONO_APP_URL` was renamed to `BETTER_AUTH_URL`, and its value is now the public **web** origin (not the API's), because auth is served same-origin through the web app's `/api` proxy. `zerostarter sync` preserves your `.env`, so rename the key there and in your deploy env. A build with `SKIP_ENV_VALIDATION` still passes, so the app only fails at boot until you do.
