...
- **`web-next`**:
  ...
  - **Client**: `NEXT_PUBLIC_APP_URL` and `NEXT_PUBLIC_API_URL` (required), `NEXT_PUBLIC_NODE_ENV`, `NEXT_PUBLIC_POSTHOG_HOST`, `NEXT_PUBLIC_POSTHOG_KEY`, `NEXT_PUBLIC_USERJOT_URL` (exposed to browser)
...
Setting `SKIP_ENV_VALIDATION=true` bypasses validation entirely, which is how CI and Docker builds run before real secrets are present. `emptyStringAsUndefined` treats empty strings as missing so optional vars fall back to their defaults.
