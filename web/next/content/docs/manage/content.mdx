...
Full-text search is powered by [Orama](https://orama.com) through Fumadocs, built by the route handler at `web/next/src/app/api/search.ts`. Press `Cmd+K` (or `Ctrl+K`) on any docs page to open it. Only the `docs` collection is indexed; the blog and the private console docs are not.
