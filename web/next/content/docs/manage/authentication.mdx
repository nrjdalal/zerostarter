...
[Better Auth](https://better-auth.com) owns the whole auth surface (sign-in methods, sessions, organizations, and roles) as one config in `packages/auth` (`schema.ts` declares the plugins and the columns they add, and the Drizzle auth schema is generated from it; `index.ts` adds the providers, cookies and hooks), backed by the same Drizzle schema as the rest of your data. It runs in-process against your own Postgres, so no user records leave your database and there is no separate auth service to keep in sync.
...
**Magic link is off by default.** The client plugin is registered but the server `magicLink` plugin is not, so the email field stays hidden rather than showing a dead control. To turn it on, register the server plugin in the `plugins` array in `packages/auth/src/schema.ts` and implement `sendMagicLink` to deliver the email:

```ts
import { magicLink } from "better-auth/plugins"

// packages/auth/src/schema.ts, inside plugins
magicLink({
  sendMagicLink: async ({ email, url }) => {
    // your own module, imported here rather than at the top of the file
...
`sendEmail` is yours to supply; any provider works (Resend, Postmark, SES). Import it inside the callback rather than at the top of `schema.ts`: that file is also what the Better Auth CLI loads, under Node, to generate the tables, so everything it imports at the top has to load there too, and a sender that reaches the database cannot (the db client imports `bun`). The client plugin is already wired, so once the server plugin is registered the sign-in dialog shows the email field and offers magic-link sign-in with no further UI change.
...
Sessions live in the `session` table and travel as a secure cookie; the active `organizationId` and `teamId` ride along on the row.

The cookie's scope is derived at build time from `HONO_APP_URL`'s public-suffix breakdown, so it follows where you deploy rather than a setting you keep in step:

| Where the api runs                                           | Cookie scope                                                                                                                                                                   |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A subdomain of a parent you own (`api.example.com`)          | A cross-subdomain `Domain` cookie, shared by web and api                                                                                                                       |
| A public hosting suffix (`*.vercel.app` and the like)        | Host-only and first-party to the web: siblings cannot share there, so the client routes sign-in same-origin through the web's `/api` proxy ([Vercel](/docs/deployment/vercel)) |
| A bare or apex host, or a bare `localhost` (as under Docker) | Host-only; on `localhost` it holds across ports                                                                                                                                |
| Local portless dev (`*.localhost`)                           | One `Domain` cookie shared by the web and api hosts. The exception to the build-time rule: the api re-derives the scope at runtime from the subdomain portless injects         |

Better Auth caches the session in a cookie for `maxAge: 300` (~5 minutes), so a role change made elsewhere can lag by that much unless the session is revoked.
