...
`sendEmail` is yours to supply; any provider works (Resend, Postmark, SES). Import it inside the callback rather than at the top of `schema.ts`: that file is also what the Better Auth CLI loads to generate the tables, with no environment and no database on hand. The client plugin is already wired, so once the server plugin is registered the sign-in dialog shows the email field and offers magic-link sign-in with no further UI change.
