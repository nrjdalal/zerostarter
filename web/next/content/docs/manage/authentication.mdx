...
Two reads deliberately skip that cache: the console page gate and the API's console middleware. A demotion or a ban has to land on the next request there, not at the end of a window. The dashboard layout keeps the cache, because it runs for every signed-in person on every render and the only thing it decides beyond the redirect is whether the Console cross-link is drawn. A ban already deletes the sessions, so the worst staleness is a demoted person seeing a link that 404s until the window expires; a fork that would rather pay a session lookup per dashboard render for an always-accurate link can add `disableCookieCache` there.
