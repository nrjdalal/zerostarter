...
The console's landing page puts the role to work: a server-driven users table backed by `GET /api/v1/admin/users`, which `adminMiddleware` gates on the same `role === "admin"` rule (403 `FORBIDDEN` otherwise). See [Data Tables](/docs/manage/data-tables).
