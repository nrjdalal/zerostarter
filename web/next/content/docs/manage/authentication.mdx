...
The `(protected)` layout (`web/next/src/app/(protected)/route.tsx`) redirects unauthenticated users server-side; API routes validate the session in the auth middleware (`api/hono/src/middlewares/auth.ts`).
