...
**Impersonation is off as a result.** It was only ever reachable through those endpoints, so `/api/auth/admin/impersonate-user` now refuses every rung; `session.impersonatedBy` still exists on the row, and the allowlist grant skips an impersonated session so acting as someone cannot quietly change their rung.
...
owner: consoleAc.newRole({ user: ["impersonate", "impersonate-admins"] }),
```
