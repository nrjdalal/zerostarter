...
Two gates enforce it. `web/next/src/lib/auth/console.ts` admits `member` and above to the pages, `notFound()`ing everyone else so the console is never advertised. The API's console middleware reads at `member` and writes at `admin` (see [API Conventions](/docs/manage/api-conventions)). Both re-read the session past the cookie cache, so a demotion or a ban takes effect on the next request, and both also refuse a banned user, which covers a ban written straight to the database rather than through the plugin.
