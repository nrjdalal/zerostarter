...
The privileged `/console` area is gated by a `console` user field (defined in `packages/auth/src/index.ts` and backed by a `console` text column on the `user` table in `packages/db/src/schema/auth.ts`). The guard in `web/next/src/lib/auth/console.ts` treats any non-empty value as full console access.

The field is declared with `input: false`, so it is **not** settable through the auth API. To grant access, set the `console` column directly on the user row (for example via `bun run db:studio`); any non-empty value works. For reference, the local agent sign-in route (`api/hono/src/routers/agents.ts`) sets `console: "admin"` on the `AgentZero` user.
