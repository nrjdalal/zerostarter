...
title: Features
description: Toggle optional surfaces (allowlist, API reference, blog, docs, internal docs, waitlist) from one config, chosen at init and flippable anytime.
---
...
| -------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `allowlist`    | Console > Access > Allowlist, `/api/v1/admin/allowlist*`, and the console grant it drives     | the API 404s, the console page renders the not-found, the nav drops it, and console access is granted per person only                               |
...
| `waitlist`     | `/waitlist` page and its API, plus Console > Waitlist > Signups and `/api/v1/admin/waitlist*` | `/waitlist` and `/api/waitlist` 404, the console page renders the not-found, the nav drops it, and a fresh fork's home becomes a plain landing page |

"Off" is not a stub: the route 404s (inside the console, as the not-found page below), the links and navigation entries disappear, and the surface is dropped from the sitemap, `llms.txt`, and search, so nothing points at a page that no longer exists.

<Callout type="info" title="Soft 404s in the console">

The console area is `force-dynamic` (its gate runs per request), and Next commits a 200 status before a page-level `notFound()` fires mid-stream, so a disabled console page renders the not-found page with an HTTP 200, not a 404. The same is true of any unknown console path, which a catch-all routes into the same boundary on purpose, so a forbidden page and a nonexistent one cannot be told apart. The area is gated and `noindex` with no content leak, and the console docs search still returns a hard 404. Every disabled page inside the console behaves this way; every disabled surface outside it returns a real 404.
...
The fork defaults are **the API reference, blog, docs, and internal docs on; the allowlist and the waitlist off** (so a fresh fork lands on a plain home). Skipping the picker keeps those defaults. `--yes` takes them without prompting.
