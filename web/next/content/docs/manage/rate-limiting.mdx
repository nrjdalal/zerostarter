...
`findIp` is called bare, with no `platform` and no trusted `proxies`, so it walks its generic header list, which its own README says to trust only behind a proxy that sets those headers. That holds on Vercel, which overwrites `x-forwarded-for`. On a deploy where Bun owns the socket with nothing in front (a bare Docker host), a client can name its own bucket by sending `x-forwarded-for`, and one that sends nothing lands on the random tier above. Put a proxy you control in front, or pass `platform` and `proxies` to `findIp` for your host.
