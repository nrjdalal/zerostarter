...
3. **IP address** (`ip:<addr>`), resolved with [`@arcjet/ip`](https://github.com/arcjet/arcjet-js): the socket peer when Bun owns the socket and the peer is a public address, which no request header can override; otherwise the client a trusted party forwarded, from Vercel's own headers on Vercel and from `x-forwarded-for` behind a reverse proxy elsewhere.
