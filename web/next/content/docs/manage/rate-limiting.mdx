...
3. **IP address** (`ip:<addr>`), resolved with [`@arcjet/ip`](https://github.com/arcjet/arcjet-js): the socket peer when Bun owns the socket and the peer is a public address, which no request header can override; otherwise the client a trusted party forwarded in `x-forwarded-for`, read last hop first. Only that header is consulted: Vercel overwrites it with the client, a reverse proxy sets it elsewhere, and the library's other fallbacks (`forwarded`, `true-client-ip`, `x-client-ip`, `x-real-ip`) are ignored, since behind a private peer they would let a client name its own bucket.
4. **Internal traffic is skipped**: a private-network peer that forwarded no client address (off Vercel, where Bun owns the socket: the web app's server-side calls, a health check, local dev behind portless) is not billed at all, since pooling it into one bucket would throttle every user at once. Front a self-host with a reverse proxy that sets `x-forwarded-for` if you want its anonymous callers limited per client. A request with neither a usable peer nor a forwarded client, which only another server adapter can produce, lands in one shared `ip:unknown` bucket rather than escaping the limit.
...
`@arcjet/ip` is a standalone header-parsing utility for finding a request's originating IP, not the Arcjet SaaS product. It needs no API key and no environment variable, and makes no network calls. It is handed the socket peer and the request's `x-forwarded-for`, nothing else, and resolves the address the IP tier keys on.
