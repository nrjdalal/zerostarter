...
3. **IP address** (`ip:<addr>`), resolved with [`@arcjet/ip`](https://github.com/arcjet/arcjet-js): the socket peer when Bun owns the socket and the peer is a public address, which no request header can override; otherwise the client a trusted party forwarded in `x-forwarded-for`, read last hop first. Only that header is consulted: Vercel overwrites it with the client, a reverse proxy sets it elsewhere, and the library's other fallbacks (`x-real-ip`, `x-client-ip`, `forwarded`) are ignored, since behind a private peer they would let a client name its own bucket.
