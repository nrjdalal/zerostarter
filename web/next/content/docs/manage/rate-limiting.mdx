...
Every response also carries the `draft-6` standard headers (`RateLimit-Limit`, `RateLimit-Policy`, `RateLimit-Remaining`, and `RateLimit-Reset`), the limiter's default, and a throttled one adds `Retry-After`, so a client can back off without guessing. The `429` is documented on every route in the OpenAPI spec, so it shows up in the [Scalar reference](/docs/getting-started/type-safe-api) too.
...
<Callout type="warn" title="The IP tier trusts forwarding headers">

`findIp` is called bare, with no `platform` and no trusted `proxies`, so it walks its generic header list. Its own README is blunt about that: the IP "should not be trusted as it can be spoofed in most cases", and the guards it offers apply only when a `platform` is passed. On Vercel the headers can be trusted anyway, because Vercel [overwrites `x-forwarded-for`](https://vercel.com/docs/headers/request-headers#x-forwarded-for) and does not forward a client-supplied one. On a deploy where Bun owns the socket with nothing in front (a bare Docker host), a client can name its own bucket by sending `x-forwarded-for`, and one that sends nothing lands on the random tier above. Put a proxy you control in front, or pass `platform` and `proxies` to `findIp` for your host.
