...
A fork scaffolded with the [CLI](/docs/getting-started/cli) inherits every skill below, rebranded, and `sync` keeps the ones it has not customized up to date.
...
| Build    | `add-package`    | Add a shared workspace package the repo's way: skeleton, `types: ["bun"]` for Bun scripts, where output goes (`.generated/` or committed source). |
...
| Operate  | `docker-test`    | Build and smoke-test the Docker images with `docker compose`, and run the golden end-to-end suite against them.                                   |
...
| Maintain | `skills-manager` | Regenerate the AGENTS.md skills tables from every skill's frontmatter, and how a fork syncs its skills from upstream.                             |
...
| Ship     | `icebox`         | Park a raised-but-undecided concern with no verdict, so the context survives instead of being forced to a call.                                   |
| Ship     | `release`        | Cut a production release by promoting `canary` to `main`; the pipeline does the version, changelog, tag, and publish.                             |

Two skills are vendored from Vercel Labs; every other skill is written for this repo. `agent-browser` (from [`vercel-labs/agent-browser`](https://github.com/vercel-labs/agent-browser)) is a discovery stub pinned in `skills-lock.json` for integrity: its committed `SKILL.md` just points the agent at the live workflow (`agent-browser skills get core`), installed with `npm i -g agent-browser && agent-browser install`. `portless` (from [`vercel-labs/portless`](https://github.com/vercel-labs/portless)) is copied in full and self-contained. Re-vendor to update either; do not hand-edit.
