...
- Make your changes, add tests if applicable, and run the checks:
  ...
  - `bun run format`
  - `bun run lint`
- Follow the repo conventions documented in `AGENTS.md` (`CLAUDE.md` is a symlink to it): use `@/` imports where applicable, never include "Co-authored-by" in commit messages, and keep comments on a single line. For UI, styling, and design decisions, follow `DESIGN.md` (surfaced by the `design` skill).
