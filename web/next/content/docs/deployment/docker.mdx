...
- **api, runtime**: its presence flips `POSTGRES_URL` from `localhost` to `host.docker.internal` (`dockerHostUrl` in `packages/env/src/lib/postgres-url.ts`, applied by `db.ts`), so the container reaches a database running on your host.
...
Both Dockerfiles share one shape, on `oven/bun:1.4.0-alpine`:
...
The API bundle does not bake `NODE_ENV` either: it is built with `bun build --env disable`, so the value `env_file` supplies at runtime decides the stage, and `/api/health` reports it as `environment`. Without the flag bun inlines whatever the build shell had, which is never what `.env` says, since the secret is a mounted file and not a sourced environment.
...
## Test the images end to end

`bun run test:e2e` drives a running stack over HTTP, and its defaults are the compose ports, so with `docker compose up` running it tests the images you just built rather than a dev server:

```bash
bun run test:e2e
```

It is a suite for a local-stage stack on a disposable database, never a deployed one: `.env` needs `NODE_ENV=local` and `AGENT_SIGNIN_ENABLED=true` so the suite can sign in, plus both OAuth client ids set to any value, since the providers response it snapshots lists them. It refuses a host that is not local, because it writes through the API. See [Scripts](/docs/getting-started/scripts#quality) for the rest, and the `docker-test` skill for the full recipe, including the offline check that proves the api bundle resolves every import without a network.
