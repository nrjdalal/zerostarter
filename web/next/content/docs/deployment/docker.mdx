...
- **web, build time**: handled by the `/api/*` proxy route (`web/next/src/app/api/$.ts`), so `/api/*` proxies to `http://api:4000`.
...
- The **web** image builds the Nitro server bundle (`web/next/.output`), and the runner starts `bun .output/server/index.mjs`.
...
Secret contents aren't part of BuildKit's cache key. Edit `.env` and a plain rebuild can reuse the cached build step, silently shipping stale values, including web's `VITE_*`, which Vite inlines into the bundle at build time. After any env change, run `docker compose build --no-cache`.
