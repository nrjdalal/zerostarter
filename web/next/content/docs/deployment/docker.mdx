...
It needs a local-stage stack on a disposable database, never a deployed one; [Scripts](/docs/getting-started/scripts#the-end-to-end-suite) lists what `.env` has to hold. The `docker-test` skill has the full recipe, including the offline check that proves the api bundle resolves every import without a network.
