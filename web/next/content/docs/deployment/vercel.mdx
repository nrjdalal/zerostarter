...
Env vars are set per project and don't propagate between them, and the two projects deploy independently. Because both live on `*.vercel.app` (a public suffix, where the browser rejects a shared `Domain` cookie), the build bakes that Public Suffix List verdict, the app detects this "split" shape from it and your URL vars, and sign-in routes through a one-time, server-to-server session handoff automatically, so the two-project setup above works with no extra configuration. When you add a custom domain, update the URL vars on both projects, redeploy both so the baked mode follows, and update the OAuth callback URLs in GitHub/Google to match your production domains.
