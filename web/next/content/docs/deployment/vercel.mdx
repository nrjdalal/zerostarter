...
`BETTER_AUTH_URL` on the API is your **web** origin (not the API's): auth is reached same-origin through the web app's `/api` proxy, so Better Auth's base URL and the OAuth callback live on the web app. `HONO_TRUSTED_ORIGINS` is the web origin too.
