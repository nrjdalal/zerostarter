...
- **A connection is pinned to one function instance** and closes when the function reaches its max duration (300 seconds by default under Fluid compute), so keep any shared socket state external (for example Redis) and let clients reconnect.
