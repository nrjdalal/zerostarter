...
`NEXT_PUBLIC_NODE_ENV` is required by the web env schema but is derived automatically from `NODE_ENV`, so you do not set it as a separate variable.
...
`HONO_PORT` exists in the env schema (default 4000) but is unused on Vercel, where the serverless runtime manages the port, so there is no need to set it.
