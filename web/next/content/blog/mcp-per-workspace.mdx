...
- It clears the tokens first and **exports only when the lookup succeeds**, so a server never receives an empty `Bearer`.
...
Nothing mutates shared state, so there's nothing to race. The one global thing left, gh's keyring "active account", only matters _outside_ your mapped directories, where the tokens are unset.
