...
![Two Claude Code sessions both switching gh's single active account, so the work session's next call goes out as the wrong user](/marketing/blog/mcp-per-workspace/images/gh-account-clash.svg)
...
![Each directory resolves to its own identity: ~/acme to the acme GitHub account, its MCP servers, and the work git email; ~/nrjdalal to the nrjdalal account, its own servers, and the personal email; with nothing shared between them](/marketing/blog/mcp-per-workspace/images/scope-by-path.svg)
...
![Flowchart: on cd, if the directory is under ~/acme resolve the acme tokens, else if under ~/nrjdalal resolve the nrjdalal tokens, otherwise leave all tokens unset](/marketing/blog/mcp-per-workspace/images/chpwd-flow.svg)
