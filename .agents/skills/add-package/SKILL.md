...
- A Bun-script package without `types: ["bun"]` fails `check-types` with `Cannot find name 'Bun'` / `'node:...'` under the native `tsc`, even though the identical library config auto-includes fine. Pin `types` for script packages only.
