...
`bun run dev` runs the stack through portless: named URLs (web `http://zerostarter.localhost:1355`, api `http://api.zerostarter.localhost:1355`; branch-prefixed in a worktree) on a shared unprivileged `:1355` HTTP proxy, with the apps on loopback `:3000`/`:4000` (each extra worktree auto-picks the next free pair, 3001/4001, ..., so several run at once). It uses `--ui tui` and needs an interactive terminal; run stream mode detached instead. `PORTLESS=0 bun run dev` bypasses portless (plain localhost).
...
```bash
WEB=http://zerostarter.localhost:1355   # the web URL `bun run dev` prints (branch-prefixed in a worktree)
