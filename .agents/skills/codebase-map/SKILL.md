...
| Gate by role | `web/next/src/lib/auth/console.ts` gates the web admin console; the API's `middlewares/auth.ts` checks the session only (401), `middlewares/admin.ts` layers the admin-role check (403) for `/api/v1/admin/*` | - |
