...
Only where the file runs **only** under Bun and the call has a `Bun.*` equivalent. Audited in
`.github/notes/audits/2026-08-23-node-to-bun-apis.md`:

| File | Node call | Bun equivalent |
| --- | --- | --- |
| `.github/scripts/ensure-remote-branches.ts` | `execFileSync` | `Bun.spawnSync` (check `exitCode`; it does not throw) |
...
`ensure-remote-branches.ts` ships to forks through `lefthook.yml`, but the hook runs it as `bun .github/scripts/...`,
so Bun is guaranteed there too. `skills-manager.ts`'s `createHash` stays on purpose: `Bun.CryptoHasher` yields the
same sha256 hex, but the helper mirrors the CLI's (Node) digest over a shared ledger, and two copies of one line beat
two implementations. Everything else on a Bun-only file is `node:path`, `node:os`, or the directory half of `node:fs`,
which Bun's own docs route to `node:` ("for operations they don't cover, such as `mkdir` or `readdir`, use `node:fs`").
