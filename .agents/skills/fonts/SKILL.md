...
- The data table measures header labels from bundled metrics of the sans woff2, generated by `.github/scripts/data-table-metrics.ts` into `.generated/data-table-metrics.json` (in the web build, dev, and check-types chains). Swapping the sans re-measures automatically on the next run, but the script's `FONT` path must point at the new file: update it when the filename changes.
