...
6. Verify rendering on the affected pages

## Notes

- The prose faces (DM Sans, Newsreader) vendor italics: `src` as an array of `{ path, style }` entries, top-level `weight` covers both. Caveat ships no italic file, and mono comments render synthetic oblique, both chosen.
