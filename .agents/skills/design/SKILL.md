...
- **Data tables:** compose the `components/data-table/` family (`DataTable`, `DataTableCellText`, `DataTableColumnHeader`, `DataTableFacetedFilter`, `DataTableToolbar`, `DataTableViewOptions`) over a page-owned table instance. Column layout is the column manager's (`components/data-table/column-manager.ts`): `COLUMN_MANAGER.<area>.<table>.<columnId>` holds `{ align?, extra?, flex?, width? }` in Tailwind spacing units mapped onto `global` archetypes (a widthless column measures its header label via pretext), applied via useDataTable's `manager` option or `applyColumnManager`; columns files carry content only (header, cell, `meta.label`). `flex` reaches back through widthless columns (explicit widths stay fixed), the last visible capable column growing, falling back to the visible widthless columns sharing growth so dead space never trails; text cells truncate with a cut-only tooltip or wrap via `DataTableCellText`; content never shifts the layout. Server-driven tables get the whole wiring (URL state, infinite query, manual-mode instance) from `useDataTable`; client tables use `useDataTableState` plus row models. Every table infinite-scrolls with virtualized rows inside `DataTable`'s region (columns declare explicit `size`, one column takes `meta.flex`; the page opts into the viewport-filling chain with `h-svh` on `PageShell` plus `flex-1 min-h-0` down to the region); no numbered pagination, and do not hand-roll table markup or table empty/loading states. Every component reading a `table`/`column` instance keeps the `"use no memo"` directive (TanStack Table v8 mutates one instance during render; the React Compiler otherwise freezes reads one render behind). Page-scoped table pieces colocate with their route in a components folder (`app/(console)/console/(platform)/users/components/data-columns.tsx` + `data-table.tsx`); only cross-page pieces live in `components/`. Reference docs: `content/docs/manage/data-tables.mdx`.
...
- Components are grouped by domain folder (`common/`, `shell/`, `console/`, `dashboard/`, `data-table/`, `docs/`, `blog/`, `marketing/`, `ui/`), with kebab-case file names. A multi-file component family gets its own folder named for the family: the namesake file matches its export (`data-table/data-table.tsx` exports `DataTable`) and the rest are role-named with family-prefixed exports (`data-table/toolbar.tsx` exports `DataTableToolbar`). Route-colocated component files are role-named the same way, with the page context carried by the export, not the filename (`users/components/data-table.tsx` exports `UsersDataTable`); the basename-matches-export rule applies to standalone single-component files in `components/`, not to family or route folders. A single-component file's basename matches its export; a multi-export slot file is named `<area>/sidebar.tsx` (console, dashboard, docs) and its exports follow the sidebar-slot rule below. `docs/` holds one of each (`docs/sidebar.tsx` + `docs/copy-as-markdown.tsx`).
