...
- Column ids say what the column shows, not the backing field (`status` over `banned`). Cells are plain text via `DataTableCellText` (truncate + tooltip by default, `wrap: true` to fold); no badges in cells for now.
- Sort UI is a plain title plus a bare icon-only sort button toggling asc/desc (icon before the title on right-aligned columns); hiding lives in `DataTableViewOptions`. Give the table a visible `defaultSorting`.
