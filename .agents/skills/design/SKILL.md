...
- Components are grouped by domain folder (`common/`, `shell/`, `console/`, `dashboard/`, `docs/`, `blog/`, `marketing/`, `ui/`), with kebab-case file names. A multi-file component family gets its own folder named for the family: the namesake file matches its export (`data-table/data-table.tsx` exports `DataTable`) and the rest are role-named with family-prefixed exports (`data-table/toolbar.tsx` exports `DataTableToolbar`). A single-component file's basename matches its export; a multi-export slot file is named `<area>/sidebar.tsx` (console, dashboard, docs) and its exports follow the sidebar-slot rule below. `docs/` holds one of each (`docs/sidebar.tsx` + `docs/copy-as-markdown.tsx`).
