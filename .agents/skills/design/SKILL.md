...
- **Forms:** native `<form>` then `<FieldGroup>` then `<form.Field>` then `<Field>` + `<FieldLabel>` + `<Input>` + conditional `<FieldError>`, with `@tanstack/react-form` + zod. Let `FieldGroup` own the vertical rhythm (no second `space-y-*`). Do not hand-roll labels or error markup. An email field carries `autoComplete="email"` and `inputMode="email"`, and points `aria-describedby` at its `<FieldError id>` while invalid. Give the schema separate messages for empty and malformed, and focus the invalid field from `onSubmitInvalid` so a failed submit does not strand focus on the button. Pass `field.state.meta.errors` straight through: `FieldError` already dedupes by message.
- **Dialogs:** bare `<DialogContent>` is centered at `sm:max-w-sm`. The auth dialog (`components/common/access.tsx`) uses `max-w-md`. Point `initialFocus` at a ref for the dialog's heading rather than leaving it default or `false`: focus must enter the dialog so the trap and the screen-reader announcement work, but landing on the first field autofocuses an input and pops the keyboard on mobile. A dialog heading is an `<h2>`; the page keeps the only `<h1>`.
