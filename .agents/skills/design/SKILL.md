...
- Toasts are raised through `toast` from `@/lib/toast`, which offers `error`, `success` and `warning` over Base UI's toast manager, and are rendered by the `Toaster` mounted in `providers.tsx`. Never import a toast library directly: the kinds this app raises are a list in that one file, and adding a fourth is an edit there rather than an options object at a call site. The surface stays neutral and the kind shows in the icon, red for an error.
