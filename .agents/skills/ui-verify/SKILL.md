...
- **Visual:** capture before and after at the same viewport (the 1728×963 default, `agent-browser set viewport 1728 963`; see the `agent-browser` skill). The "before" is the pre-change state: `git stash` (or check out the pre-change commit), `agent-browser screenshot before.png`, then restore your change and `agent-browser screenshot after.png`.
