...
- The typed client reaches it with `wsApiClient.health.ws.$ws()` (not `apiClient`), a standard `WebSocket` at the API's own origin (`http` becomes `ws`). Browser RPC is same-origin through the `/api` proxy, but a WebSocket can't ride it, so `wsApiClient` stays cross-origin at the API origin.
