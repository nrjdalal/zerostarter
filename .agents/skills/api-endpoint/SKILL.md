...
- Declare the gate in the document too: `security: sessionSecurity` on an auth route, `security: consoleSecurity` on an admin route, both from `@/lib/openapi`, next to the error-response spreads. The console one names the minimum role in the requirement, which OpenAPI 3.1 allows for non-OAuth schemes; a public route carries no `security`.
