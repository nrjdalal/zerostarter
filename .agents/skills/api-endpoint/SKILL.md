...
- On the pinned `hono-openapi`, its `validator` is used deliberately over `@hono/standard-validator`'s `sValidator`. An older version once regressed Hono RPC inference (the typed client), which is why validation lived on `sValidator` for a while; on the current pin the two produce a byte-identical `AppType` and identical runtime validation (verified end-to-end). Keep the guarantee live: when you bump `hono-openapi`, rebuild and diff `api/hono/dist/index.d.mts` against the prior build. If the typed client (the crown jewel) changed, the bump regressed inference, revert to `sValidator("json", schema, hook)` for validation and document the `requestBody` via `jsonRoute` instead.
