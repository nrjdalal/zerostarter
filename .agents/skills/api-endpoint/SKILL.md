...
- Admin-only routes go in `admin.ts` (mounted at `/v1/admin`), behind `adminMiddleware`, which 403s unless `user.role === "admin"`; spread `...forbiddenErrorResponses` into their responses. `GET /api/v1/admin/users` is the reference, including server-driven list conventions (page/perPage, whitelisted `sort` + `dir`, escaped `LIKE` search, value filters).
