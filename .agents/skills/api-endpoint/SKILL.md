...
- Spread `codeSample(...)` from `routers/admin/shared.ts` so Scalar shows the `hono/client` usage (the template above omits it). It writes the `x-codeSamples` block and its cast, so a route supplies only the snippet.
...
- Reference for a list endpoint: the users list route in `routers/admin/users.ts` (whitelisted `sort` union with nullable columns coalesced, `LIKE` wildcards escaped, an `asc(user.id)` tiebreaker, `page`/`perPage` batching answered with `pagingFields`, and an explicit NULLS LAST where a sortable column is nullable).
