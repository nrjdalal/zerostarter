...
- ALWAYS: Keep enumerable lists alphabetical (A→Z), union/enum members, env-var schemas and their `runtimeEnv`/`turbo.json` mirrors, and the docs that list them, so the code and its docs stay in the same order. Use a meaningful order (flow, priority, required-then-optional) only where alphabetical would obscure intent. Two API shapes take that exemption deliberately: a row leads with its `id` and then runs A→Z, and a paged payload names its collection before the paging fields, which are themselves in reading order (`page`, `perPage`, `total`, `hasNextPage`). Alphabetical there splits an envelope's meaning across three endpoints, which is the drift this rule exists to prevent.
