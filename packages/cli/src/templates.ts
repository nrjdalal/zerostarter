...
// A fresh fork's default surfaces: the API reference, blog, docs, and internal docs on; the waitlist off, so the home is a plain landing page, and the allowlist off, so console access is granted per person. Any can be flipped later in the config.
